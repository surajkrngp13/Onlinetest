...
console.log("auto8TabMode")
 ...
 let btns = document.querySelectorAll("#tabbox button");
let btn = btns[tabIndex] || null;
...
const tabKeys = [...new Set(exam2D.map(r => r[0]))];
tabList = tabKeys.map(name => name.replace(/[^a-zA-Z0-9_]/g, "_"));
...
function startTotalTimer() {
    ...
    totalTimerInterval1 = setInterval(() => {
        if (totalSeconds1 > 0 && !examSubmitted1) {
            ...
            submitExamFinal(); // 🔥 fallback auto submit
        }
    }, 1000);
}
...
function escapeHtml(str) {
    ...
    self.onmessage = function(e) {
        ...
        if (type === 'generateDetailedHTML') {
            const { questions, userAnswers,markedForReview, questionTimeSpent, userAnswerTexts, correctAnswerTexts, tabWiseHtml } = data;
            let html = '';
...
function showDetailedAnswers() {
  ...
  const tabWiseHtml = (() => {
    let html = "<h3>📊 Tab Wise Result</h3>";

    const allTabs = tabList.length ? tabList : Object.keys(tabResults);

    allTabs.forEach(tab => {
        let r = tabResults[tab] || {
            correct: 0,
            wrong: 0,
            marks: 0,
            time: 0
        ...
        html += `
            <div style="margin:6px 0;padding:8px;border:1px solid #ddd;border-radius:6px;">
                <b>${tab}</b><br>
                ✅ ${r.correct} | ❌ ${r.wrong}<br>
                🧮 Marks: ${r.marks}<br>
                ⏱️ Time: ${r.time}s
            </div>
        `;
    ...
    <div class="detailed-card">
        ...
        <div style="margin-top: 20px; text-align: center;">
            <button id="saveObjectionsBtn" class="btn btn-warning">💾 Save All Objections</button>
        </div>

        <button class="close-detailed" id="closeDetailedBtn">Close</button>
    </div>
`;
...
function getTabKey(idx) {
  return exam2D[idx][0].replace(/[^a-zA-Z0-9_]/g, "_");
}

function updateTabResult() {
  let tabKey = getTabKey(currentQuestionIndex1);
  ...
  for (let i = 0; i < exam2D.length; i++) {
    ...
    let userAns = userAnswers1[i];
    let correctAns = exam2D[i][7];

    let qMarks = Number(exam2D[i][11]) || 0;
    let qTime = Number(exam2D[i][12]) || 0;

    if (userAns !== null) {
      if (userAns === correctAns) result[key].correct++;
      else result[key].wrong++;
    }

    // always add marks & time (attempted + unattempted both optional logic below)
    if (typeof autoIndividualMarksAddition !== "undefined" && autoIndividualMarksAddition) {
      result[key].marks += qMarks;
      result[key].time += qTime;
    } else {
      // fallback: only attempted questions
      if (userAns !== null) {
        result[key].marks += qMarks;
        result[key].time += qTime;
      }
...
async function saveReportCard() {
  ...
  const line =
    `${window.examName || "Exam"}¦${encrypted}`;

  let oldData = await window.getfilefromindexd("reportcard.txt");
  if (!oldData) oldData = "";

  const updated = oldData + "\n" + line;

  await window.savefileinindexd(updated, "reportcard.txt");
}



function generateResultSummaryText() {
  let correct = 0;
  let wrong = 0;

  for (let i = 0; i < exam2D.length; i++) {
    if (userAnswers1[i] !== null) {
      if (userAnswers1[i] === exam2D[i][7]) correct++;
      else wrong++;
    }
  }

  let total = exam2D.length;
...
Total: ${total}
Time: ${formatTime(totalSeconds1)}
`;
}

function parseReportLine(line){
  const [exam, encrypted] = line.split("¦");
  const [ts, salt, iv, data] = encrypted.split("¦");

  return { exam, ts, salt, iv, data };
}
...
function renderTabSummary() {
  let box = document.getElementById("tabResultBox");
  if (!box) {
    box = document.createElement("div");
    box.id = "tabResultBox";
    document.getElementById("tabbox").after(box);
  }

  let html = "<div style='display:flex;gap:10px;margin-top:5px;'>";
...
function updateTotalTimerDisplay() {
    ...
    if (totalSeconds1 <= 0 && timerEnabled1 && !examSubmitted1) {
        ...
        submitExamFinal(); // 🔥 AUTO SUBMIT HERE
    }
}
...
async function showResultsSummary() {
    ...
    let tabWiseHtml = "<h3>📊 Tab Wise Result</h3>";
...
function renderCurrentQuestion() {
    ...
    const explanation = row[8];
     const Qfeedback = row[9];  // it is passage  const rowPassage = exam2D[currentQuestionIndex1][9];
      const Qnotranslationset = row[10];
       const Qmarks = row[11]; 
       const Qtime = row[12];
   
    const qDivId = "q" + (currentQuestionIndex1+1);
  const noTranslate = Qnotranslationset?.toUpperCase() === "N" ? 'translate="no"' : "";
    ...
    const html = `
        ...
        <div class="question-card" id="${qDivId}" ${noTranslate}>
            <div class="question-header">
                <div class="qno">Question ${currentQuestionIndex1+1} of ${totalQuestions}</div><div> ${Qmarks}</div><div> ${Qtime}</div>
